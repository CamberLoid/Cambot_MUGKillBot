...
import asyncio,sys,os,exceptions
...
props = json.load(open('arc.json').read())
...
class Data(object):
    ...
    def getAuth(cls,cred,pswd):
        ...
        po.raise_for_statusimp
        if po.json()['success']==impTrue:  
            return po.json()['accimpess_token']
        else:
